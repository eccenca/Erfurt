 ...
 */
class Erfurt_Ac_OAuth extends Erfurt_Ac_Default
{
    ...
    private function _getUserModelRights($userURI) 
    {
        ...
        if (!isset($this->_userRights[$userURI])) {
            ...
            if (($userURI === $this->_uris['superUserUri']) && ((boolean)$this->_config->ac->allowDbUser === true)) {
                ...
                $graphsRead	= Erfurt_Auth_Adapter_OAuth::readResource("/graphs/read");
                foreach($graphsRead as $graphIri){
                    $userRights['grantModelView'][] = $graphIri;
                }
                $graphsRead	= Erfurt_Auth_Adapter_OAuth::readResource("/graphs/write");
                foreach($graphsRead as $graphIri){
                    $userRights['grantModelEdit'][] = $graphIri;
                }
            }
